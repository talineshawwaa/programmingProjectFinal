...
int analyze_react(struct ParseTree *node, struct SymbolTable **table);
...
const char* type2str(int type){
    switch (type){
        case _int: return "protons";
        case _float: return "electrons";
        case _string: return "alkali";
...
int analyze_Num(struct ParseTree *tree) {
    ...
    if (num->data->type != electrons)
        // it's the sign
        num = num->sibling;
    if (num->child->data->type != protons)
        return _float;
...
int analyze_Obj(struct ParseTree *tree, struct SymbolTable **table, struct Symbol **sym) {
    ...
    if (node->data->type == LISTELEMENTS)
...
int analyze_BreakLine(struct ParseTree *node, struct SymbolTable **table, struct ContextStack **stack) {
    ...
    if ((*stack)->top != react && (*stack)->top != ReactBody)
